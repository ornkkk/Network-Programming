...
void drop_client(struct client_info *client) {
    /*
    closes the connection to a client and removes it from the
    clients linked list.
    */
...
const char *get_client_address(struct client_info *ci) {
    /*
    returns a client's IP address as a string (character
    array).
    */
...
void send_400(struct client_info *client) {
    /*
    used to handle HTTP error conditions.
    */
...
int main(int argc, char *argv[]) {
    ...
    //SOCKET server = create_socket("127.0.0.1", "8080"); //If you want to accept connections from only the local system, and not outside systems
    while(1) {
        fd_set reads;
